...
class Publications extends Component {
  ...
  render() {
    ...
    if (this.props.publication.length > 0) {
      lastSearchedAll = this.props.publication.reduce((a, b) =>
        a.last_searched > b.last_searched ? a : b
      ).last_searched;
    }
    if (typeof lastSearchedAll === "string") {
      readableTime =
        lastSearchedAll.slice(0, 10) + " " + lastSearchedAll.slice(12, -5);
    }
      ...
      <>
        ...
        >
          <Typography variant="h6">
            Total Publications Uploaded:{" "}
            <Typography variant="body1" component="span">
              {this.props.publication.length}
            </Typography>
          </Typography>
          <Typography variant="h6">
            Last Search Date:
            <Typography variant="body1" component="span">
              {/* {JSON.stringify(this.props.publiction)} */} {readableTime}
            </Typography>
          </Typography>
          <Typography variant="h6">
            Searches Remaining:{" "}
            <Typography variant="body1" component="span">
              {this.props.user.rate_limit_remaining}
            </Typography>
          </Typography>
          <Typography variant="h6">
            Unsearched Publications:{" "}
